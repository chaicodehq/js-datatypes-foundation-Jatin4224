...
export function getItemNames(items) {
  ...
  const c = items.map((item) => item.name);
  return c;
}
...
export function getAffordableItems(items, maxPrice) {
  ...
  const c = items.filter((item) => item.price <= maxPrice);
  return c;
}
...
export function calculateTotal(items) {
  ...
  const c = items.reduce((total, item) => {
    let sum = total + item.price * item.qty;
    return sum;
  }, 0);
  return c;
}
...
export function sortByPrice(items, ascending) {
  ...
  if (ascending) {
    const c = [...items].sort((a, b) => a.price - b.price);
    return c;
  } else {
    const c = [...items].sort((a, b) => b.price - a.price);
    return c;
  }
}
