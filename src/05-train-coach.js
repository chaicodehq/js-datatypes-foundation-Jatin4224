...
export function isAnyWaitlisted(passengers) {
  ...
  const c = passengers.some((passenger) => passenger.status === "waitlisted");
  return c;
}
...
export function areAllConfirmed(passengers) {
  ...
  //  *
  if (!Array.isArray(passengers) || passengers.length === 0) return false;
  const c = passengers.every((passenger) => passenger.status === "confirmed");
  return c;
}
...
export function getWaitlistedPassengers(passengers) {
  ...
  const c = passengers.filter((passenger) => passenger.status === "waitlisted");
  return c;
}
